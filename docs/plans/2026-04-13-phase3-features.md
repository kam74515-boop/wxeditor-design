...
### 后端新建文件:
1. `server/src/repositories/articleBatch.repo.js` - 批次文章数据层
2. `server/src/services/articleBatch.service.js` - 批次文章业务逻辑
3. `server/src/controllers/articleBatch.ctrl.js` - 批次文章路由
4. `server/migrations/003_article_batches.js` - 数据库迁移
5. `server/src/routes/index.js` - 注册路由
