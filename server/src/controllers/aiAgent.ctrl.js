...
router.post('/refine', async (req, res) => {
  try {
    const { content, action, model } = req.body;
    ...
    const stream = await AiAgentService.refineContent(content, action, model);
    pipeSSEStream(stream, res);
  } catch (err) {
...
router.post('/title', async (req, res) => {
  try {
    ...
    const result = await AiAgentService.optimizeTitle(title, model);
