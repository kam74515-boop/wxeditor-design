...
const { createChatMessages, getActiveConfig, getPrompt, parseToolCall, getToolDefinitions } = require('../../src/ai');
...
jest.mock('../../src/ai', () => ({
  ...
  getToolDefinitions: jest.fn(() => [{ type: 'function', function: { name: 'replace_editor_content' } }]),
}));
jest.mock('../../src/services/aiModelCatalog.service', () => ({
  resolveRequestedModel: jest.fn(async (config, requestedModel) => requestedModel || config.model),
}));
...
describe('pipeSSEStream', () => {
  ...
  it('should ignore [DONE] signal until stream end', () => {
    pipeSSEStream(mockStream, mockRes);
  ...
  it('should accumulate tool_calls and emit on end', async () => {
    parseToolCall.mockReturnValue({ tool: 'search', args: { q: 'test' } });
    ...
    expect(mockRes.write).toHaveBeenCalledWith(
      expect.stringContaining('"tool":"search"')
    );
    expect(mockRes.end).toHaveBeenCalled();
  });

  it('should end response on stream end without tool calls', async () => {
    pipeSSEStream(mockStream, mockRes);
    handlers.end();
    await new Promise((resolve) => setImmediate(resolve));
  ...
  it('should handle stream error', () => {
    ...
    expect(mockRes.write).toHaveBeenCalledWith(
      expect.stringContaining('"message":"Connection lost"')
    );
    expect(mockRes.end).toHaveBeenCalled();
