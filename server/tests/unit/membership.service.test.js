...
jest.mock('../../src/services/productCatalog.service', () => ({
  getCatalog: jest.fn(),
}));
...
describe('MembershipService', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    ProductCatalogService.getCatalog.mockResolvedValue([
      { code: 'basic', enabled: true, prices: { monthly: 29, quarterly: 79, yearly: 199 }, features: ['基础功能'] },
      { code: 'pro', enabled: true, prices: { monthly: 99, quarterly: 269, yearly: 799 }, features: ['高级功能'] },
      { code: 'enterprise', enabled: true, prices: { monthly: 299, quarterly: 799, yearly: 1999 }, features: ['企业功能'] },
    ]);
  });

  // ── getPlans ─────────────────────────────────────
  describe('getPlans', () => {
    it('should return plans and currency', async () => {
