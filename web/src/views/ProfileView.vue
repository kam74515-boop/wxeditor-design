<template>
  <WorkspaceLayout :menu-items="workspaceMenuItems">
    ...
    <template #sidebar-footer>
      <button class="workspace-layout__sidebar-action" @click="router.push('/projects')">
    ...
    <div class="profile-view">
      <header class="page-header">
        <div class="page-heading">
          <h2 class="page-title">{{ activeTab === 'profile' ? '个人资料' : '安全设置' }}</h2>
          <div class="profile-tabs">
            <button
              :class="['profile-tab', { 'profile-tab--active': activeTab === 'profile' }]"
            ...
            <button
              :class="['profile-tab', { 'profile-tab--active': activeTab === 'security' }]"
...
function setActiveTab(tab: 'profile' | 'security') {
  activeTab.value = tab;
  const targetPath = tab === 'security' ? '/settings' : '/profile';
...
watch(
  () => route.path,
  (path) => {
    activeTab.value = path === '/settings' ? 'security' : 'profile';
  },
  { immediate: true }
);
</script>
