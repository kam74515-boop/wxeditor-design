<template>
  ...
  <div>
    <div class="admin-main__header">
      ...
      <div style="display: flex; gap: 8px;">
        <button
          ...
          @click="tab = t.value"
        >{{ t.label }} ({{ t.value === 'pending' ? pendingTotal : t.value === 'approved' ? approvedTotal : rejectedTotal }})</button>
      </div>
    </div>
    ...
    <div style="display: flex; gap: 12px; margin-bottom: 16px;">
      ...
      <el-select v-model="contentType" placeholder="全部类型" clearable size="small" style="width: 140px;" @change="fetchContent">
    ...
    <table class="admin-table" v-loading="loading">
      ...
      <tbody>
        <tr v-for="item in contentList" :key="item.id">
          ...
          <td v-if="tab === 'history'" style="color: rgba(0,0,0,0.5);">{{ item.reviewerName || '-' }}</td>
          <td>
          ...
          <td>
            <div style="display: flex; gap: 6px;">
              ...
              <el-button v-if="item.status !== 'pending'" size="small" text @click="openRejectDialog(item)">详情</el-button>
            </div>
          </td>
        </tr>
      </tbody>
    ...
    <div v-if="!loading && contentList.length === 0" style="text-align: center; padding: 60px 20px; color: rgba(0,0,0,0.3);">
      ...
      <p>暂无{{ tab === 'pending' ? '待审核' : tab === 'approved' ? '已通过' : '已拒绝' }}内容</p>
    </div>
...
async function fetchCounts() {
  try {
    ...
    // 使用 mock
    pendingTotal.value = 12;
    approvedTotal.value = 156;
    rejectedTotal.value = 8;
  }
}
...
async function fetchContent() {
  loading.value = true;
  try {
    const status = tab.value === 'history' ? '' : tab.value;
    ...
    // Mock 数据
    contentList.value = generateMockContent();
...
function generateMockContent(): ContentItem[] {
  const types = ['article', 'template', 'comment'];
  const statuses = tab.value === 'history'
    ? ['approved', 'rejected']
    : [tab.value];
  const titles = [
    '关于2026年春节放假安排的通知',
  ...
  return Array.from({ length: 8 }, (_, i) => ({
    ...
    content: '<p>这是模拟内容，用于展示审核功能。实际内容将由用户提交。</p><p>第二段文字，包含更多详细信息。</p>',
    type: types[i % types.length],
    status: statuses[i % statuses.length],
    authorName: authors[i % authors.length],
    reviewerName: tab.value === 'history' ? '管理员' : undefined,
...
async function batchReject() {
  if (!selectedItems.value.length) return;
  try {
    await ElMessageBox.confirm(`确定要批量拒绝 ${selectedItems.value.length} 条内容吗？`, '批量拒绝', { type: 'warning' });
    await http.post('/admin/content-review/batch-reject', { ids: selectedItems.value });
